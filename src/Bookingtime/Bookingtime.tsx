...
export const Bookingtime = () => {
  ...
  const [selectedTheater, setSelectedTheater] = useState<string | null>(
    "2D Vietnam Sub",
  );
  ...
  const handleTheaterClick = (index: number) => {
    ...
    setSelectedTheater(theater !== selectedTheater ? theater : null);
  };

  return (
    <div
      id="cboxContent"
      className="mt-5 box-content h-[37rem] w-[84rem] overflow-x-auto overflow-y-hidden bg-[#222] text-sm"
    >
      <div
        id="cboxLoaded"
            ...
            <div className="border-[#222]bg-transparent relative flex flex-col  border-y-[1.6px] bg-origin-padding p-5 text-center sm:flex-row sm:flex-wrap">
              {MovieFormats.map((format, index) => (
                <div
                  ...
                  onClick={() => handleTheaterClick(index)}
                >
                  {format}
                </div>
              ))}
